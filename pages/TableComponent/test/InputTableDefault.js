...
function InputTableDefault({
  ...
  const takeInput = (e, id, type, iDecimalPlaces, form) => {
    ...
    if (typeof iDecimalPlaces == "object") {
      ...
      if (name1 == "sConversion" && (priceSelect == "sPriceOption1" || priceSelect == "sPriceOption2") && value1) {
  ...
  if (data?.component?.options?.mode === "DEFAULT") {
    columns.unshift({
      ...
      renderCell: params => {
        const index = (csvFile && csvFile.length > 0) ? optimizedCSVRows.findIndex((row) => row.id === params.row.id) + 1 : rows.findIndex((row) => row.id === params.row.id) + 1;
        return <Typography
          sx={{
            height: "100%", 
            display: "flex", 
  ...
  function summaryFieldsCalculation() {
    setSummaryForBackend(prevState =>
      prevState.map(item =>
  ...
  //       if(apiRef && apiRef.current && apiRef.current?.updateRows){
  ...
  async function maptoRow() {
    ...
    transformedData.forEach(async (item) => {
      ...
      if (qtyCodeIndex != -1) {
        ...
        console.log("jwbfjkewbejw", filteredQtyCode, item.id)
      }
    })
    let updatedRowData = [];
    transformedData.forEach((row, index) => row.id = maxId + index);
  ...
  function summaryFieldsFilteration() {
      ...
      {csvFile && csvFile.length > 0 ?
        <div style={{ ...data?.component?.sParentContainerProps?.style }}>
          <DataGridPro
            ...
            rowBuffer={100}
          />
        </div> : formAction === "EDIT" ? <Box sx={{ position: 'relative' }}>
          <div style={{ ...data?.component?.sParentContainerProps?.style }}>
            <DataGridPro
              getRowId={(row) => row.id}
              loading={false}
